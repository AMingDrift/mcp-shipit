...
server.registerTool(
    "download_from_github_release",
    {
        ...
        description:
            "Downloads a specified file from a GitHub Release and extracts it to a target directory.",
        inputSchema: {
            projectRootDir: z
                .string()
                .describe("The absolute path to the project root directory."),
            ...
            filename: z
                .string()
                .describe(
                    "The name of the file to download from GitHub Release."
                ),
            mode: z
    ...
    async ({ projectRootDir, targetDir, filename, mode }) => {
        // 动态导入 mcpUnshipit 函数
        const { mcpUnshipit, setLoggingCallback } = await import(
            "./utils/index.js"
        );
        ...
        try {
            ...
            const { extractPath } = await mcpUnshipit(
                projectRootDir,
                targetDir,
                cleanFilename,
                mode
