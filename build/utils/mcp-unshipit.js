...
import { downloadFromGithubRelease, getOrCreateRelease } from "./github-release.js";
 ...
 */
export async function mcpUnshipit(projectRootDir, targetDir, filename, mode = "overwrite") {
    try {
        // 清理文件名，去除回车、换行、空格等无效字符
        const cleanFilename = filename.trim().replace(/[\r\n\t]/g, "");
        logMessage(`Starting mcpUnshipit process for target directory: ${targetDir}`, "info");
        // 步骤1: 检查环境变量
        ...
        const downloadedFilePath = await downloadFromGithubRelease(projectRootDir, cleanFilename);
        ...
        logMessage(`Extracting to directory: ${destDir} with mode: ${mode}`, "info");
        await unzipToDirectory(downloadedFilePath, destDir, mode);
        logMessage(`Extraction completed to: ${destDir}`, "info");
        // 删除下载的zip文件
        logMessage(`Cleaning up temporary file: ${downloadedFilePath}`, "debug");
...
//# sourceMappingURL=mcp-unshipit.js.map